...
/*
    ...
    */
    ...
    fun restaurantDetailAPICall2(restaurantID: String): YelpRestaurantDetail? {
        var apiResponse: YelpRestaurantDetail? = null
        ...
        call.enqueue(object : Callback<YelpRestaurantDetail> {
            override fun onResponse(
                ...
                Log.w(TAG, apiResponse?.transactions.toString())
            }

            override fun onFailure(call: Call<YelpRestaurantDetail>, t: Throwable) {
                Log.i(TAG, "onFailure $t")
                throw Exception("API called failed")
            }
        })
        return apiResponse
    }

    suspend fun restaurantResultAPICall(
        categoryInput: String,
        locationInput: String,
        restaurantNameInput: String,
    ) = suspendCoroutine<YelpSearchResult> {
        val retroInstance = RetroInstance.getRetroInstance()
        val yelpService = retroInstance.create(YelpService::class.java)
        val call = yelpService.getRestaurantsResults(
            "Bearer $API_KEY",
            restaurantNameInput,
            categoryInput,
            locationInput
        ...
        call.enqueue(object : Callback<YelpSearchResult> {
            ...
            override fun onResponse(
                ...
                Log.i(
                    "$TAG RestaurantResultAPICall",
                    "makeAPICall onResponse on call $response"
                )
                it.resume(response.body() as YelpSearchResult)
